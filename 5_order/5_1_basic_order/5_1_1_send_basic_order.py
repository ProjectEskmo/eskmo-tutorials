...
def onOrderSendStart(data):
    ...
    # OrderSendStartResult(account='91829808465', symbol='2888', exchange=0, period=0, order_flag=0, buysell=0, price=7.5, qty=1, price_type=0, trade_type=2, callbackId=0)
    
@api.event.order.send_fail
def onOrderSendFail(data):
    print(f"Order send fail: {data}")
    # OrderSendFailResult(account='91829808465', symbol='2888', exchange=0, period=0, order_flag=0, buysell='yoyo', price=-7.5, qty=-1, price_type=0, trade_type=2, callbackId=0, errors=["無效送單: Failed to prepare inputs before executing 'SendStockOrder': invalid literal for int() with base 10: 'yoyo'."], error_code=None)
    
@api.event.order.send_success
def onOrderSendSuccess(data):
    print(f"Order send success: {data}")
    # OrderSendSuccessResult(account='91829808465', symbol='2888', exchange=0, period=0, order_flag=0, buysell=0, price=-7.5, qty=-1, price_type=0, trade_type=2, state='Pending', created=datetime.datetime(2024, 4, 16, 16, 42, 50, 663000), callbackId=0, threadId='38448')

@api.event.order.placed_success
def onOrderPlacedSuccess(data):
    print(f"Order placed success: {data}")
    # OrderPlaceSuccessResult(threadId='9728', order=PlacedOrderResult(account='91829808465', symbol='2888', exchange=0, period=0, order_flag=0, buysell=0, price=7.5, qty=1, price_type=0, trade_type=2, seqNo='4304171180151')

@api.event.order.placed_fail
def onOrderPlacedFail(data):
    print(f"Order placed fail: {data}")
    # OrderPlaceFailResult(threadId='14196', order={'pid': 26940, 'bstrFullAccount': '91829808465', 'bstrStockNo': '2888', 'sPrime': 0, 'sPeriod': 0, 'sFlag': 0, 'sBuySell': 0, 'bstrPrice': -7.5, 'nQty': -1, 'nTradeType': 0, 'nSpecialTradeType': 2, 'OrderState': 'Pending', 'APICallbackID': 0, 'StartTime': 1713258953597, 'OrderFrom': 'BasicOrder', 'PendingNewThreadID': '14196', 'ThreadID': '14196'}, errors=["ThreadID=14196 委託不明, 可能功能尚未實作 (未知nCode=106): 'price -7.5 error 13493'"], error_code=106)
